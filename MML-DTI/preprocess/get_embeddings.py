...
def get_data_list(data_path):
    ...
    for no,data in enumerate(full.values):
        smiles,protein,interaction =data
        if interaction==1:
            pos+=1
        else:
            neg+=1
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset_name', type=str, default="drugbank", choices=["drugbank","KIBA","Davis"],help='select dataset for training')
    ...
    save_path = os.path.join('../dataset',args.dataset_name)

    get_data_list(data_path)
    get_protein_embeddings(data_path,args.dataset_name,save_path,args.device)

    get_smiles_embeddings(data_path, args.dataset_name,save_path, args.device)
